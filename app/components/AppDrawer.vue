...
const library = computed(() => [
  { title: $t('Favourites'), icon: mdiHeartOutline, to: localePath('/library/favourites') },
  { title: $t('Watchlist'), icon: mdiBookmarkOutline, to: localePath('/library/watchlist') },
  { title: $t('History'), icon: mdiHistory, to: localePath('/library/history') },
])
...
<template>
  ...
  >
    <nuxt-link :to="localePath('/')" class="flex items-center gap-3 px-4 py-5">
      <img src="/logo.svg" alt="Rivulet" class="size-26px shrink-0">
      <span v-if="!rail" class="text-title-large whitespace-nowrap font-bold tracking-tight">Rivulet</span>
    </nuxt-link>
