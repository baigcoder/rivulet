...
<template>
  <div v-if="channels.length" class="px-4 md:px-6">
    <h3 class="mb-2 text-title-small font-medium opacity-70">
      {{ $t('Favorites') }}
    </h3>
    <div class="flex gap-3 overflow-x-auto pb-2 scrollbar-none">
      <button
        ...
        :key="ch.id"
        type="button"
        class="group relative w-28 shrink-0 cursor-pointer overflow-hidden rounded-xl border border-white/5 bg-surface-container-high transition-[border-color,box-shadow] hover:border-primary/40 hover:shadow-lg hover:shadow-primary/10 focus-visible:border-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary sm:w-32"
        @click="emit('play', ch)"
      >
        <div class="aspect-square w-full overflow-hidden bg-surface-container">
          <img
            v-if="ch.logoUrl"
