...
// question: `reduce-effects` is about what costs a repaint (blur, frosted
// chrome), this is about what is allowed to move. A television wants the first
// off and can afford the second — see MotionMode in stores/settings.
//
// Three tiers rather than a boolean, and only one of them is on `<html>` at a
// time, so a stale class from a previous value can't leave two sets of tokens
// fighting.
watchEffect(() => {
  const html = document.documentElement
  for (const tier of ['full', 'reduced', 'none'] as const)
    html.classList.toggle(`motion-${tier}`, settings.effectiveMotion === tier)
})
