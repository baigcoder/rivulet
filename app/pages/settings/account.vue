 ...
 */
const FILE = 'rivulet-backup.json'
...
<template>
  <div class="flex flex-col gap-8">
    <settings-section
      :title="$t('Sync')"
      :hint="$t('Rivulet keeps your library on this device. Carrying it to another screen is the backup file below.')"
    >
        ...
        <v-card v-if="summary" rounded="xl" :title="$t('Restore this backup?')">
          <v-card-text class="flex flex-col gap-3">
            ...
            <p v-if="summary.sources" class="text-body-small opacity-60">
              {{ $t('Restoring changes which servers Rivulet searches — a backup carries its own source list, and it wins.') }}
            </p>
