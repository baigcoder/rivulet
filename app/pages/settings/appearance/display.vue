...
<template>
  <div class="flex flex-col gap-6">
    ...
    <settings-section
      :title="$t('Motion')"
      :hint="$t('How much moves as things appear and as you move around.')"
    >
      <settings-segment v-model="settings.motion" :options="MOTION_MODES" inline />
      <p class="text-body-medium opacity-70">
        {{ $t('Automatic follows the device: less motion on a television, and less again if your system already asks for reduced motion everywhere. Reduced keeps things appearing but stops them travelling to get there. Off is instant, and loading spinners keep turning either way.') }}
      </p>
      <p class="text-body-medium opacity-70">
        {{ $t('This is separate from the performance switch below on purpose. That one turns off blur, which is expensive to draw; the movement here is built out of the two things a graphics chip does almost for free, so a television can usually keep it.') }}
      </p>
    </settings-section>
    ...
    >
      ...
      <p class="text-body-medium opacity-70">
        {{ $t('It tells most while moving around with a remote, which is where the fades stack up: on the television this was measured on, that went from 3 frames a second to 23. It starts on for a television, and off on a phone or a computer.') }}
      </p>
    </settings-section>
