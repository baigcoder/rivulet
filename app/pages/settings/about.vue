...
<template>
  <div class="flex flex-col gap-8">
    <settings-section title="Rivulet">
    ...
    <settings-section :title="$t('Updates')">
      ...
      <template v-if="updates.available">
        <p class="text-body-medium">
          {{ $t('Rivulet {version} is out', { version: updates.available.version }) }}
        </p>
      ...
      <template v-else>
        <p class="text-body-medium opacity-70">
          {{ updates.current ? $t('Rivulet is up to date.') : $t('Updates are checked in the installed app, not here.') }}
        </p>
        <div>
          <v-btn
            :prepend-icon="mdiUpdate"
    ...
    <settings-section :title="$t('Legal')">
      <p class="text-body-medium">
        {{ $t('Rivulet hosts no content, indexes no content, and ships with no sources configured. It is a BitTorrent client with a player attached: it fetches only what you point it at, from servers you added yourself.') }}
      </p>
      <p class="text-body-small opacity-70">
      ...
      <p class="text-body-small opacity-70">
        {{ $t('Rivulet is MIT licensed. The components it is built on keep their own terms, listed above; on Windows the bundled mpv is GPL software and its licence and offer of source sit next to the application\'s executable.') }}
      </p>
    </settings-section>
  </div>
</template>
