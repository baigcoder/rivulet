...
export const useLibraryStore = defineStore('library', () => {
   ...
   * titleKey.
   */
  const media = useLocalStorage<Record<string, Media>>(key('media'), {})
  /** Keyed by progressKey — one entry per movie and per episode. */
  const progress = useLocalStorage<Record<string, Progress>>(key('progress'), {})
   ...
   */
  const favourites = useLocalStorage<Record<string, number>>(key('favourites'), {})
  const watchlist = useLocalStorage<Record<string, number>>(key('watchlist'), {})
