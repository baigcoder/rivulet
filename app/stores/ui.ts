...
export const useUiStore = defineStore('ui', () => {
  const layout = useLocalStorage<Layout>(key('layout'), 'grid-detail')
  ...
  const cardWidth = useLocalStorage(key('cardWidth'), 170)
  /** Desktop: collapsed icon-only sidebar. */
  const rail = useLocalStorage(key('rail'), false)
  /** Mobile: the sidebar is an overlay, so it needs an open/closed state. */
  const drawer = ref(false)

  /**
   * A source URL that arrived on a `rivulet://` link and is waiting to be
  ...
  const blur = useLocalStorage(key('blur'), 44)
  const tint = useLocalStorage(key('tint'), 0.92)

  /** Where the art behind the app comes from, if anywhere. */
  const backdropMode = useLocalStorage<BackdropMode>(key('backdropMode'), 'art')
