...
<img src="public/logo.svg" width="112" alt="Rivulet">
...
[Why Rivulet](#why-rivulet) &middot; [Features](#feature-tour) &middot; [Sources](#sources) &middot;
...
> [!IMPORTANT]
> **Rivulet hosts no content and indexes no content.** It ships with **no sources**, searches
> nothing on its own, and will not suggest anywhere to look. A source is a URL *you* add — see
> [Sources](#sources). With none configured, Rivulet is a general-purpose torrent client with a
...
shows. Rivulet embeds the real thing and keeps the library around it honest and local.
...
- **Your library stays yours.** History, progress, favourites and the watchlist live in this
  device's storage. No Rivulet account, no Rivulet server, no sync — one file carries the lot.
...
There are 26 presets beside it, dark and light — Rivulet Dark, Midnight (OLED), Carbon, Monochrome,
Nord, Tokyo Night, Mocha, Dracula, Rosé Pine, Gruvbox, Monokai, Solarized, Ember, Forest, Abyss,
Crimson, Amethyst, Rivulet Light, Paper, Latte, Frost, Mint and more: each is that same generated
...
Rivulet can search for a title if you tell it where to look. **It does not come with anywhere to
...
That's the whole integration. Rivulet runs no code from a source — it reads the JSON, ranks the
...
A stream can name a `url` instead of an `infoHash`, and Rivulet plays that directly. That's what a
...
budget, and win the last tiebreak within a quality tier. Rivulet holds no debrid credentials
...
`…/manifest.json` address; Rivulet trims it to the base it needs and keeps any configuration the
addon put in the path.

### Adding a source by link

Rivulet registers the `rivulet://` URL scheme, so a page can offer a one-click install the way
addon pages already do for other players:

```html
<a href="rivulet://your-addon.example.com/manifest.json">Add to Rivulet</a>
```

The app opens (or comes to the front), shows the URL in full, and **asks before adding it** — a
link can never change what Rivulet searches on its own. Under *Settings → Sources* there's also a
switch to handle `stremio://` links, since addon pages publish those rather than `rivulet://` ones.

Only one app can own a scheme. On Linux, Rivulet takes `stremio://` on first run **only if nothing
...
in this device's `localStorage`. There is no Rivulet account and no Rivulet server, and nothing is
...
Save a backup* writes `rivulet-backup.json` into your documents folder (a browser session downloads
...
Besides the sources you add and the peers your torrents connect to, Rivulet reaches:
...
| **GitHub** (`api.github.com`) | One request at startup asking whether a newer Rivulet has been released — the public releases endpoint, unauthenticated, sending nothing but the request itself. |
...
Rivulet checks once at startup whether this project has published a newer release, and puts a
...
open it, usually with *"Rivulet is damaged and can't be opened. You should move it to the Trash."*

Nothing is damaged. That message is what Gatekeeper says about a downloaded app when no paid
certificate vouches for it. Drag Rivulet to Applications, then clear the quarantine flag the
download arrived with:

```bash
xattr -dr com.apple.quarantine /Applications/Rivulet.app
```
...
`bun run build:windows` cross-compiles a working `rivulet.exe` — cargo-xwin supplies the MSVC
...
```bash
adb logcat -s Rivulet:V chromium:V RustStdoutStderr:V   # app + webview console + Rust
...
next to `Rivulet.exe` — which also means the raw `.exe` only plays if the `mpv/` folder travels
...
```mermaid
flowchart TD
  subgraph Window["Rivulet window"]
...
**Is Rivulet affiliated with Stremio?**
...
**macOS says Rivulet is damaged. Is it?**
...
> [!IMPORTANT]
> Rivulet is a general-purpose BitTorrent client and media player. It hosts nothing, indexes
...
[MIT][LICENSE] © Rivulet contributors.

Rivulet's own source only. Components it ships or depends on keep their own terms — in particular
...
<br/><br/>
<sub>Built to be used from a sofa. &middot; <a href="#readme-top">&#9650; back to top</a></sub>
</div>

<!-- links -->

[releases]: https://github.com/baigcoder/rivulet/releases
[issues]: https://github.com/baigcoder/rivulet/issues
[license]: LICENSE
