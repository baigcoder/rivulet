...
const live = fakeStore({
  [`${PREFIX}media`]: JSON.stringify({ 'movie:603': { title: 'The Matrix' }, 'tv:1396': { title: 'Breaking Bad' } }),
  [`${PREFIX}progress`]: JSON.stringify({ 'movie:603': { position: 10, duration: 100, at: 1, watched: false } }),
  [`${PREFIX}favourites`]: JSON.stringify({ 'tv:1396': 5 }),
  [`${PREFIX}watchlist`]: JSON.stringify({ 'movie:603': 6, 'tv:1396': 7 }),
  [`${PREFIX}sources`]: JSON.stringify(['https://a.example']),
  [`${PREFIX}theme`]: '"dark"',
...
const target = fakeStore({ [`${PREFIX}media`]: '{}', [`${PREFIX}uiScale`]: '1.4' })
applyBackup(round, target)
assert.equal(target.data[`${PREFIX}media`], live.data[`${PREFIX}media`], 'replaced')
assert.equal(target.data[`${PREFIX}uiScale`], '1.4', 'a key the backup never mentions survives')
...
const hostile = readBackup(JSON.stringify({
  ...
  version: 1,
  at: 1,
  keys: { [`${PREFIX}theme`]: '"light"', 'token': 'nope', 'vuetify:dynamic-theme': 'nope' },
}))
assert.deepEqual(Object.keys(hostile.keys), [`${PREFIX}theme`])
...
for (const [text, why] of [
  ...
  ['{"app":"rivulet","version":9,"keys":{"rivulet.theme":"1"}}', 'a format this build cannot read'],
  ['{"app":"rivulet","version":1,"keys":{}}', 'nothing to restore'],
] as const)
