...
;(globalThis as any).RivuletPlayer = { codecs: () => JSON.stringify(['audio/eac3', 'video/hevc']) }
...
assert.equal(normalizeSource('rivulet://a.example/manifest.json'), 'https://a.example')
assert.equal(normalizeSource('rivulet://a.example/opt=x,y/manifest.json'), 'https://a.example/opt=x,y')
...
for (const scheme of ['rivulet', 'stremio'])
