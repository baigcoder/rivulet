...
assert.match(libSrc, /fn play_url_mpv/, 'Linux trailers play in a system mpv window, not a YouTube iframe')
assert.match(libSrc, /fn apply_bundled_ytdlp/, 'mpv --ytdl must see the bundled yt-dlp, not the AppImage PATH')
assert.match(libSrc, /fn is_youtube_watch/, 'play_url_mpv must refuse anything but a YouTube watch URL')
