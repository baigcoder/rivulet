...
function launch(options: {
  ...
  const sandbox: any = {
    ...
    localStorage: {
      ...
      getItem(key: string) {
        ...
        return (key === 'rivulet.ground') ? options.remembered ?? null : null
      },
    },
  ...
  return {
    boot: sandbox.__rivuletBoot,
    panel: () => body.children.find((c: any) => c.id === 'rivulet-boot-error') ?? null,
    hint: () => body.children.find((c: any) => c.id === 'rivulet-boot-hint') ?? null,
...
// hint reads its own contrast off whichever ground it ended up with.
{
  const dark = launch()
  dark.advance(3000)
  assert.match(wordmark(dark.hint()), /#6FE3FF/i, 'the brand cyan on the grounds it was drawn for')

  const light = launch({ remembered: '#eee9e9' })
  light.advance(3000)
  assert.ok(!wordmark(light.hint()).toLowerCase().includes('#6fe3ff'), 'and never on a light one, where it falls under 3:1')
}
