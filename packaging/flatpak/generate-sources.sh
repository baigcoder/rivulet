...
#
#   TMDB_API=... ./generate-sources.sh v0.2.3 ~/src/flathub-rivulet
#
...
# source sits at the same indentation and was getting Rivulet's revision.
sed -e "s|__TMDB_READ_TOKEN__|$TMDB_API|" \
    -e "s|__TAG__|$tag|" \
    -e "s|__COMMIT__|$commit|" \
    "$here/io.github.rivulet.Rivulet.yml" > io.github.rivulet.Rivulet.yml
...
sed -e "s|<release version=\"[^\"]*\" date=\"[^\"]*\"/>|<release version=\"$version\" date=\"$(git -C "$root" log -1 --format=%ad --date=short "$tag")\"/>|" \
    "$here/io.github.rivulet.Rivulet.metainfo.xml" > io.github.rivulet.Rivulet.metainfo.xml
appstreamcli validate --explain io.github.rivulet.Rivulet.metainfo.xml

cp "$here/io.github.rivulet.Rivulet.desktop" "$here/flathub.json" .

echo
echo "✓ $out"
ls -la io.github.rivulet.Rivulet.yml io.github.rivulet.Rivulet.metainfo.xml \
       io.github.rivulet.Rivulet.desktop flathub.json \
       cargo-sources.json node-sources.json package-lock.json

if [ -n "$flathub" ]; then
	cp io.github.rivulet.Rivulet.yml io.github.rivulet.Rivulet.metainfo.xml \
	   io.github.rivulet.Rivulet.desktop flathub.json \
...
echo "  flatpak-builder --force-clean --user --install builddir $out/io.github.rivulet.Rivulet.yml"
echo "  flatpak run io.github.rivulet.Rivulet"
