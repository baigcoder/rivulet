...
- **macOS is not a child window, it is libmpv in-process.** The platform embeds
  ...
  film is up — so macOS uses Android's compositing (`behind`, the `rivulet-video`
...
- Playback starts through `downloads.start(key, …)`, never `startTorrent`
  ...
  (`rivulet.cached`), and that map is what lets an already-downloaded film play
...
- *Only download on Wi-Fi* (`settings.wifiOnly`) is enforced in the downloads
  ...
  (`navigator.connection` can't tell) — `metered()` on the `RivuletScreen` bridge,
...
- `app/utils/backup.ts` carries every `rivulet.` localStorage key by design, so a
...
- **A white screen is a bug report with nothing in it.** The bundle is built for
  ...
  Every error also lands on `window.__rivuletBoot` for adb and devtools. Keep
...
- **One colour, four files, none of which can import the other three.** Three
  ...
  `utils/ground.ts` writes whatever is actually on screen to `rivulet.ground` and
...
- **No language in the URL** (`strategy: 'no_prefix'`), so a path is just a
  ...
  remembered in `rivulet.locale` and restored in `app.vue` — not in this module's
...
Rivulet searches only the servers a user adds under *Settings → Sources*. That is
...
- `rivulet://` deep links stage a source and **always ask before adding it**
