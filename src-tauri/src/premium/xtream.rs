...
impl IPTVProvider for XtreamAdapter {
    ...
    /// and nothing logs it.
    ///
    /// MPEG-TS is the original FHD/4K HEVC feed. The `.m3u8` sibling is
    /// a transcoded HLS ladder (often 720p/1080p H.264) so a browser
    /// can play it — opening that on mpv/VLC is why a channel named 4K
    /// looked like HD. A panel that only has HLS answers `.ts` with a
    /// redirect, and the proxy retries the other extension if this 404s.
