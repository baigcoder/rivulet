...
// processes at all, so it plays through libVLC instead, which needs no Rust:
// the frontend talks to it over the same JavascriptInterface bridge the
// activity already had (`VlcPlayer.kt`).
//
...
// rectangle — though on Android nothing asks, since `hasVlcPlayer()` picks the
