...
/// Sharper than mpv's default hermite downscale. 4K into a 1080p window
/// otherwise looks like a soft HD upscale even when the decode is UHD.
/// Mitchell + linear-downscaling is physically nicer and looks like a
/// 720p upscale — lanczos without linear light keeps the UHD bite.
#[cfg_attr(target_os = "macos", allow(dead_code))]
pub fn picture_cli() -> &'static [&'static str] {
	&[
		"--vd-lavc-threads=0",
		"--scale=lanczos",
		"--cscale=lanczos",
...
#[cfg_attr(not(target_os = "macos"), allow(dead_code))]
pub fn picture_kv() -> &'static [(&'static str, &'static str)] {
	&[
		("vd-lavc-threads", "0"),
		("scale", "lanczos"),
		("cscale", "lanczos"),
