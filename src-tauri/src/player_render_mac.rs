...
//     be see-through down to the video box (`html.rivulet-video` in
...
pub fn install(
    mpv_ctx: NonNull<mpv_handle>,
    ns_window_ptr: isize,
    geometry: Geometry,
) -> Result<(), String> {
    let mtm =
        MainThreadMarker::new().ok_or("the video surface must be built on the main thread")?;
    if ns_window_ptr == 0 {
    ...
    unsafe {
        ...
        let content_view = ns_window
            .contentView()
            .ok_or("the app window has no content view")?;
        ...
        let view: Option<Retained<NSOpenGLView>> = msg_send![NSOpenGLView::alloc(mtm), initWithFrame: content_view.bounds(), pixelFormat: &*pf];
...
pub fn render_now() {
    ...
    unsafe {
        let Some(gl_ctx) = embed.view.openGLContext() else {
            return;
        };
        gl_ctx.makeCurrentContext();
        let as_view: &NSView = embed.view.as_super();
        ...
        let Ok(render) = embed.render.lock() else {
            return;
        };
