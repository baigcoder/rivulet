...
pub async fn stream_redirect(
    ...
    if let Some((kind, id, ext)) = XtreamAdapter::parse_vod_play_id(&channel_id) {
        ...
        let proxied = crate::iptv::commands::proxy_free_stream_url(
            upstream,
            Some(crate::iptv::proxy::IPTV_PLAYER_UA.to_string()),
            None,
        );
        return Ok(Redirect::temporary(&proxied).into_response());
    }
    ...
    // Loopback proxy, not the panel: mpv then talks to :3031 (range
    // fallback, .ts/.m3u8 swap) and never sits on each Xtream 302 hop.
    // A player UA, not Chrome: the panel's HTML5 ladder is why a 4K
    // channel looked like 720p even after we asked for `.ts`.
    let ua = stored
        .user_agent
        .filter(|s| !s.is_empty())
        .or_else(|| Some(crate::iptv::proxy::IPTV_PLAYER_UA.to_string()));
    let proxied = crate::iptv::commands::proxy_free_stream_url(
        upstream,
        ua,
        stored.referer,
    );
    Ok(Redirect::temporary(&proxied).into_response())
}
