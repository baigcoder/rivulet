...
fn on_main<R: Send + 'static>(
    ...
    rx.recv_timeout(Duration::from_secs(5))
        .map_err(|_| "the main thread never answered".to_string())
}
...
pub fn player_start(
    ...
    let ns_window = window
        .ns_window()
        .map_err(|e| format!("no NSWindow: {e}"))? as isize;

    let mut player = state.0.lock().unwrap();
    // The handle existing is "running" — idle-active is true while the
    // first pieces arrive, and treating that as dead would kill FileStream.
    if player_direct::reuse_engine_process(player.url.as_deref(), &url, player.mpv.is_some()) {
        return Ok(());
    }
    player.stop(&app);
    ...
    let mpv = Mpv::with_initializer(move |init| {
        ...
        set("cache", "yes");
        // keep-open already keeps libmpv alive through the first empty
        // read. cache-pause-initial waits for a second of timeline before
        // the first frame — on HEVC that is a whole GOP, the "starts at
        // 1%" wait. Start as soon as a frame decodes.
        set("cache-pause-initial", "no");
        if local_file {
            for (k, v) in player_direct::file_kv() {
                set(k, v);
            }
        } else {
        ...
        if !local_file {
            set(
                "stream-lavf-o",
                player_direct::stream_lavf_o(
                    engine_stream,
                    live.unwrap_or(false),
                    seekable.unwrap_or(false),
                ),
            );
        }
        set("keep-open", if engine_stream { "yes" } else { "no" });
    ...
    mpv.set_property("vo", "libmpv")
        .map_err(|e| format!("vo=libmpv refused: {e:?}"))?;
    ...
    on_main(&app, move || {
        match std::ptr::NonNull::new(ctx as *mut libmpv2_sys::mpv_handle) {
...
pub fn player_set_geometry(
    ...
    let _ = on_main(&app, move || {
        player_render_mac::resize_to(geometry, visible)
    });
}
...
pub fn player_status(state: tauri::State<'_, PlayerState>) -> PlayerStatus {
    ...
    let log_tail = if running {
        ...
        player.log.as_deref().and_then(player_socket::log_tail)
    };
